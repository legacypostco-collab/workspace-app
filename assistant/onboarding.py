    ...
    if step == "verified":
        ...
        kpi_items = [
            ...
            {"label": _("Поведенческая (40%)"), "value": f"{round(behavioral_score)}/100",
             ...
             "sub": _("Сроки · сделки · рекламации")},
            {"label": _("Соблюдение сроков"), "value": f"{sla_pct:.0f}%" if n_total else "—",
        ...
        if not kyb.phone:
            boosters.append({"title": _("📞 Телефон срочной связи · обязателен для подбора с подтверждением"),
                              "action": "update_kyb_contacts", "params": {"focus": "phone"}})
        if not kyb.warehouse_address:
            boosters.append({"title": _("📍 Адрес склада · покупатель видит ожидаемый срок на этапе подбора"),
        ...
        if n_breached > 0:
            boosters.append({"title": _("⏱ Нарушений срока: %(n)s · каждое снижает поведенческую оценку на %(pen)s")
        ...
        if next_tier and n_delivered < 100:
            boosters.append({"title": _("📈 До +30 баллов за объём · сейчас +%(now)s из 30, отвечайте на большее число заявок") % {"now": round(beh_volume_pts)},
                              "action": "seller_inbox", "params": {}})

        if not boosters:
            boosters.append({"title": _("🎉 Все базовые условия выполнены · соблюдайте сроки и закрывайте сделки — рейтинг будет расти."),
                              "action": "seller_inbox", "params": {}})
        ...
        tips = [
            {
                "title": _("🤖 Автоподбор — указывайте только фактическое наличие"),
                "subtitle": _("Платформа подбирает поставщика по каталогу. Срыв такого заказа критичен: наличие было подтверждено вашим прайсом. Два-три несовпадения снижают уровень надёжности."),
            },
            {
                "title": _("🎯 Подбор с проверкой — отвечайте оператору в первые 15 минут"),
                "subtitle": _("Оператор подобрал аналог из вашего каталога и ждёт подтверждения. При долгом молчании заявка перейдёт другому поставщику."),
            },
            {
                "title": _("📋 Прайс-лист — 16 фиксированных колонок, не отклоняйтесь"),
                "subtitle": _("Номер детали, бренд, наличие, срок подготовки, цена FOB и страна происхождения обязательны для автоподбора. Строки с неполными данными попадут на проверку."),
            },
            {
                "title": _("⏱ Резерв 10% оплачен → у вас 24 часа на подтверждение"),
                "subtitle": _("Это переход от поступления денег к оформлению заказа. При просрочке резерв вернётся покупателю, а оценка соблюдения сроков снизится."),
            },
            {
                "title": _("🏭 Обновляйте статус производства не реже одного раза в неделю"),
                "subtitle": _("Покупатель следит за этапами заказа. После семи дней без изменений оператор автоматически запросит состояние поставки. Короткого обновления раз в два-три дня достаточно."),
            },
            {
            ...
            },
            {
                ...
                "subtitle": _("После этого статуса начинается отсчёт ожидаемого срока. Устанавливайте его только после фактической готовности груза к погрузке."),
            },
            {
            ...
            },
            {
                ...
                "subtitle": _("Внешняя оценка основана на юридических данных и меняется редко. Поведенческая зависит от сроков, объёма и рекламаций. Для роста рейтинга закрывайте сделки без нарушений."),
            },
            {
                "title": _("🚫 Прямой контакт с покупателем в обход чата запрещён"),
                "subtitle": _("Платформа фиксирует попытки увести сделку. Первый случай приводит к предупреждению, повторный — к блокировке. Обсуждайте сделку в чате заказа."),
            },
            {
                "title": _("🔄 Эскалация спора — через оператора, не через рекламацию"),
                "subtitle": _("Если покупатель недоволен, сначала свяжитесь с оператором. Если вопрос не решён, открывайте рекламацию. Преждевременная рекламация снижает рейтинг."),
            },
        ]
