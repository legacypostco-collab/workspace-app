...
def _draw_footer(c):
    ...
    c.drawCentredString(
        width / 2, 10 * mm,
        "Consolidator Parts · marketplace@consolidatorparts.com · "
...
def _proforma_storage_path(rfq_id: int, quote_id: int, buyer_id: int) -> str:
    return f"proforma_invoices/{buyer_id}/PRO-RFQ{rfq_id}-Q{quote_id}.pdf"


def _save_proforma_pdf(rfq, quote, buf: io.BytesIO) -> str:
    """Store a private pro-forma PDF and return its authorized endpoint."""
    from django.core.files.base import ContentFile
    from django.core.files.storage import default_storage

    storage_path = _proforma_storage_path(rfq.id, quote.id, rfq.created_by_id)
    buf.seek(0)
    if default_storage.exists(storage_path):
        default_storage.delete(storage_path)
    default_storage.save(storage_path, ContentFile(buf.read()))
    return f"/api/assistant/rfq/{rfq.id}/quotes/{quote.id}/proforma.pdf"
...
class ProformaInvoicePdfView(APIView):
    """Serve a stored pro-forma only to its buyer or an operator."""

    permission_classes = [IsAuthenticated]

    def get(self, request, rfq_id, quote_id):
        from django.core.files.storage import default_storage
        from django.http import FileResponse, Http404

        from marketplace.models import Quote, RFQ
        from .permissions import detect_user_role

        rfq = RFQ.objects.filter(id=rfq_id).first()
        quote = Quote.objects.filter(id=quote_id, rfq_id=rfq_id).first()
        if not rfq or not quote:
            raise Http404("proforma not found")
        role = detect_user_role(request.user, request=request)
        if (
            rfq.created_by_id != request.user.id
            and role != "admin"
            and not role.startswith("operator")
        ):
            raise Http404("proforma not found")

        storage_path = _proforma_storage_path(
            rfq.id,
            quote.id,
            rfq.created_by_id,
        )
        ...
        response = FileResponse(
            ...
            as_attachment=True,
            filename=f"PRO-RFQ{rfq.id}-Q{quote.id}.pdf",
...
def generate_invoice_pdf(params, user, role):
    ...
    if role == "seller":
        ...
        if len(seller_ids_for_order(order)) > 1:
            return ActionResult(
                text=_(
                    "В заказе участвуют несколько поставщиков. Общий счёт "
                    "формирует оператор."
                ),
            )
    try:
...
def generate_packing_list_pdf(params, user, role):
    ...
    if role == "seller":
        ...
        if len(seller_ids_for_order(order)) > 1:
            return ActionResult(
                text=_(
                    "В заказе участвуют несколько поставщиков. Общий "
                    "упаковочный лист формирует оператор."
                ),
            )
    try:
...
def generate_qc_report_pdf(params, user, role):
    ...
    if role == "seller":
        ...
        if len(seller_ids_for_order(order)) > 1:
            return ActionResult(
                text=_(
                    "В заказе участвуют несколько поставщиков. Общий акт "
                    "контроля качества формирует оператор."
                ),
            )
    try:
    ...
    try:
        ...
        for it in OrderItem.objects.filter(order=order).select_related("part__seller"):
            if it.part and it.part.seller_id and it.part.seller:
                ...
                if seller_can_access_document(it.part.seller, doc):
                    recipients.setdefault(
                        it.part.seller_id,
                        (it.part.seller, _("Продавец")),
                    )
