...
def op_payments_dashboard(params, user, role):
    ...
    rules_faq = {
        ...
        "data": {
            ...
            "items": [
                {"q":    _("Что платит клиент (по базису поставки)"),
                 "rows": pricing_items},
                {"q":    _("Условия оплаты клиентом (10 / 80 / 10)"),
                 "rows": payment_terms_items},
                {"q":    _("Что получает поставщик и как работает депозит"),
                 "rows": seller_payout},
                {"q":    _("🏆 Тарифные уровни (скидка по обороту)"),
                 "rows": tier_items},
            ],
        },
    }
