        ...
        cards=[
            ...
            {"type": "faq", "data": {
                ...
                "items": [
                    ...
                    {"q": _("Счета и банковские платежи"),
                     "rows": [
                        {"title": _("Красная"), "subtitle": _("Счёт просрочен или операция отменена")},
                        {"title": _("Оранжевая"), "subtitle": _("Платёж заявлен и ожидает сверки")},
                        {"title": _("Зелёная"), "subtitle": _("Банковская операция подтверждена")},
                     ]},
                    {"q": _("Рейтинг и уровни поставщика"),
                     "rows": [
                    ...
                    {"q": _("Режимы обработки заявок"),
                     "rows": [
                        {"title": _("Красная"), "subtitle": _("Срок обработки оператором нарушен (15 минут или 48 часов)")},
                        {"title": _("Оранжевая"), "subtitle": _("В работе у оператора, идёт обратный отсчёт")},
                        {"title": _("Голубая"), "subtitle": _("Автоматический подбор, проверка оператора не требуется")},
                     ]},
                ],
            }},
        ],
...
FAQ_ENTRIES = [
    ...
    },
    {
        ...
        "a": (_("Двухэтапная оплата по договору и банковским счетам:\n\n"
              "1. После подтверждения КП платформа формирует договор с покупателем "
              "и первый счёт на 10%. Заказ начинается после сверки поступления "
              "финансовым оператором.\n\n"
              "2. Окончательный счёт на 90% выставляется после готовности к "
              "отгрузке. Отдельные закупочные договоры и счета между платформой "
              "и продавцами не раскрывают данные покупателя.")),
    },
    {
        "category": _("Заказы и оплата"),
        "q": _("Можно ли отменить заказ?"),
        "a": (_("До подтверждения первого платежа — да, из карточки заказа.\n\n"
              "После подтверждения первого платежа (10%) — только через оператора. "
    ...
    },
    {
        ...
        "a": (_("• Полный возврат банковским переводом\n"
              "• Частичный возврат или компенсация за дефект\n"
    ...
    },
    {
        ...
        "a": (_("Нет, никогда. Платформа намеренно не раскрывает контакты "
              ...
              "Это защищает обе стороны: у покупателя есть журнал событий и "
              "договор с платформой, у поставщика — отдельный закупочный договор. "
    ...
    },
    {
        ...
        "q": _("Что умеет помощник в чате?"),
        "a": (_("• OEM-нормализатор: 707-99-58030, 7079958030, CAT-265-0235 — "
              "находит одну и ту же деталь под любым написанием\n"
              "• Загрузка спецификации: перенос файлов xlsx, csv и pdf прямо в чат, "
              "распознавание колонок с названием, номером и количеством\n"
              "• Подбор поставщиков: автоматический, с подтверждением или ручной — "
              "система выбирает режим по сложности запроса\n"
              "• Полный расчёт: цена, доставка и таможня по выбранным условиям поставки\n"
              "• Отслеживание с прогнозом: текущий этап и ожидаемая дата выдачи")),
    },
    {
        "category": _("Платформа"),
                ...
                "data": {
                    ...
                    "fields": [
                        ...
                        {"name": "text", "label": _("Опишите проблему"),
                         ...
                         "placeholder": _("Что произошло, в каком заказе или заявке, что хотите получить")},
                    ],
