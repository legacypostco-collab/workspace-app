...
class Command(BaseCommand):
    help = "Import Sandvik XLSX price list."
    ...
    def handle(self, *args, **options):
        ...
        brand = Brand.objects.filter(name__iexact="Sandvik").first()
        if not brand:
            brand = Brand.objects.create(name="Sandvik", slug="sandvik", region="europe")
            ...
            if to_update:
                with transaction.atomic():
                    Part.objects.bulk_update(
                        to_update,
                        ["title", "description", "price", "stock_quantity", "condition", "image_url", "category", "brand", "is_active"],
        ...
        for row in rows[1:]:
            ...
            part_number = (data.get("C") or "").strip()
            if not part_number:
                skipped_no_part += 1
                continue
            name = (data.get("B") or "").strip() or f"Part {part_number}"
            price = _decimal_or_none(data.get("E"))
            if price is None:
                skipped_no_price += 1
                continue
            delivery = (data.get("G") or "").strip()
            weight = (data.get("H") or "").strip()
            notes = (data.get("I") or "").strip()
            description = " | ".join(part for part in [name, f"Delivery: {delivery}" if delivery else "", f"Weight: {weight}" if weight else "", notes] if part)
        ...
        self.stdout.write(
            self.style.SUCCESS(
                f"Sandvik import done. Created: {created}, Updated: {updated}, "
