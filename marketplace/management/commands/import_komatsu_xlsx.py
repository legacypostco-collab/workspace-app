import re
import zipfile
import xml.etree.ElementTree as ET
from decimal import Decimal
from pathlib import Path
from uuid import uuid4

from django.contrib.auth.models import User
from django.core.management.base import BaseCommand, CommandError
from django.db import transaction
from django.utils.text import slugify

from marketplace.models import Brand, Category, Part

NS_MAIN = {"x": "http://schemas.openxmlformats.org/spreadsheetml/2006/main"}
NS_REL = {"r": "http://schemas.openxmlformats.org/package/2006/relationships"}
REL_NS = "{http://schemas.openxmlformats.org/officeDocument/2006/relationships}"


def _col_letters(cell_ref: str) -> str:
    m = re.match(r"([A-Z]+)", cell_ref or "")
    return m.group(1) if m else ""


def _decimal_or_none(value) -> Decimal | None:
    if value is None:
        return None
    raw = str(value).strip()
    if not raw:
        return None
    raw = raw.replace(",", "").replace(" ", "")
    try:
        d = Decimal(raw).quantize(Decimal("0.01"))
        return d if d > 0 else None
    except Exception:
        return None


def _load_shared_strings(z: zipfile.ZipFile) -> list[str]:
    if "xl/sharedStrings.xml" not in z.namelist():
        return []
    root = ET.fromstring(z.read("xl/sharedStrings.xml"))
    values: list[str] = []
    for si in root.findall("x:si", NS_MAIN):
        text = "".join((t.text or "") for t in si.findall(".//x:t", NS_MAIN))
        values.append(text)
    return values


def _sheet_path(z: zipfile.ZipFile, index: int = 0) -> str:
    wb = ET.fromstring(z.read("xl/workbook.xml"))
    sheets = wb.find("x:sheets", NS_MAIN)
    if sheets is None or len(list(sheets)) == 0:
        raise CommandError("No sheets found in workbook.")
    target_sheet = list(sheets)[index]
    rid = target_sheet.attrib.get(f"{REL_NS}id")
    rels = ET.fromstring(z.read("xl/_rels/workbook.xml.rels"))
    rel_map = {r.attrib["Id"]: r.attrib["Target"] for r in rels.findall("r:Relationship", NS_REL)}
    if rid not in rel_map:
        raise CommandError("Cannot resolve worksheet path.")
    return "xl/" + rel_map[rid]


def _cell_text(cell: ET.Element, shared: list[str]) -> str:
    t = cell.attrib.get("t")
    v = cell.find("x:v", NS_MAIN)
    if v is None:
        return ""
    raw = (v.text or "").strip()
    if t == "s" and raw.isdigit():
        idx = int(raw)
        return shared[idx] if 0 <= idx < len(shared) else raw
    return raw


def _row_map(row: ET.Element, shared: list[str]) -> dict[str, str]:
    out: dict[str, str] = {}
    for c in row.findall("x:c", NS_MAIN):
        ref = c.attrib.get("r", "")
        col = _col_letters(ref)
        if not col:
            continue
        out[col] = _cell_text(c, shared)
    return out


class Command(BaseCommand):
    help = "Import Komatsu XLSX price list (supports Office XML .xlsx without openpyxl)."

    def add_arguments(self, parser):
        parser.add_argument("--xlsx", required=True, help="Absolute path to xlsx file")
        parser.add_argument("--seller", default="seller", help="Seller username/email")
        parser.add_argument("--category", default="Рабочее оборудование", help="Target category")
        parser.add_argument("--batch-size", type=int, default=1000, help="Batch size")
        parser.add_argument("--default-stock", type=int, default=20, help="Default stock")

    def handle(self, *args, **options):
        xlsx_path = Path(options["xlsx"]).expanduser()
        if not xlsx_path.exists():
            raise CommandError(f"File not found: {xlsx_path}")

        seller_ref = (options["seller"] or "").strip()
        seller = User.objects.filter(username=seller_ref).first() or User.objects.filter(email__iexact=seller_ref).first()
        if not seller:
            raise CommandError(f"Seller not found: {seller_ref}")

        category_name = options["category"].strip()
        category = Category.objects.filter(name=category_name).first()
        if not category:
            category = Category.objects.create(name=category_name, slug=slugify(category_name)[:140] or f"cat-{uuid4().hex[:8]}")

        brand = Brand.objects.filter(name__iexact="Komatsu").first()
        if not brand:
            brand = Brand.objects.create(name="Komatsu", slug="komatsu", region="europe")

        shared: list[str]
        rows_iter = []
        with zipfile.ZipFile(xlsx_path) as z:
            shared = _load_shared_strings(z)
            sheet_xml_path = _sheet_path(z, index=0)
            root = ET.fromstring(z.read(sheet_xml_path))
            sheet_data = root.find("x:sheetData", NS_MAIN)
            if sheet_data is None:
                raise CommandError("No sheet data found.")
            rows_iter = list(sheet_data.findall("x:row", NS_MAIN))

        if not rows_iter:
            raise CommandError("No rows found.")

        header_map = _row_map(rows_iter[0], shared)
        # Expected by file sample:
        # A=件号 (part number), D=名称 (name), F=成本, H=促销价格, I=sp价格
        if "A" not in header_map:
            raise CommandError("Cannot detect part number column in xlsx.")

        existing_by_oem = {p.oem_number: p for p in Part.objects.filter(seller=seller).only("id", "oem_number").iterator(5000)}
        to_create: list[Part] = []
        to_update: list[Part] = []
        batch_size = int(options["batch_size"])
        default_stock = max(0, int(options["default_stock"]))

        created = 0
        updated = 0
        skipped_no_price = 0
        skipped_no_part = 0

        def flush():
            nonlocal created, updated
            if to_update:
                with transaction.atomic():
                    Part.objects.bulk_update(
                        to_update,
                        [
                            "title",
                            "description",
                            "price",
                            "stock_quantity",
                            "condition",
                            "image_url",
                            "category",
                            "brand",
                            "is_active",
                        ],
                        batch_size=batch_size,
                    )
                updated += len(to_update)
                to_update.clear()
            if to_create:
                with transaction.atomic():
                    Part.objects.bulk_create(to_create, batch_size=batch_size)
                created += len(to_create)
                to_create.clear()

        for row in rows_iter[1:]:
            m = _row_map(row, shared)
            part_number = (m.get("A") or "").strip()
            if not part_number:
                skipped_no_part += 1
                continue

            name = (m.get("D") or "").strip() or f"Part {part_number}"
            price = _decimal_or_none(m.get("I")) or _decimal_or_none(m.get("H")) or _decimal_or_none(m.get("F"))
            if price is None:
                skipped_no_price += 1
                continue

            description = name
            existing = existing_by_oem.get(part_number)
            if existing:
                existing.title = name[:255]
                existing.description = description
                existing.price = price
                existing.stock_quantity = default_stock
                existing.condition = "oem"
                existing.image_url = "/static/marketplace/komatsu-logo.svg"
                existing.category = category
                existing.brand = brand
                existing.is_active = True
                to_update.append(existing)
            else:
                base = slugify(f"{part_number}-{name}")[:220] or "part"
                to_create.append(
                    Part(
                        seller=seller,
                        title=name[:255],
                        slug=f"{base}-{uuid4().hex[:8]}",
                        oem_number=part_number,
                        description=description,
                        price=price,
                        stock_quantity=default_stock,
                        condition="oem",
                        image_url="/static/marketplace/komatsu-logo.svg",
                        category=category,
                        brand=brand,
                        is_active=True,
                    )
                )

            if len(to_update) >= batch_size or len(to_create) >= batch_size:
                flush()

        flush()

        self.stdout.write(
            self.style.SUCCESS(
                f"Komatsu import done. Created: {created}, Updated: {updated}, "
                f"Skipped no price: {skipped_no_price}, Skipped no part: {skipped_no_part}"
            )
        )
